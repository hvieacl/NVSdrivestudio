...
conda run -n "%SEGFORMER_ENV_NAME%" python waymo_workflow\semantic_masks\cli.py extract --data_root "%DATA_ROOT%" --scene_ids "%SCENE_IDS%" --segformer_path "%SEGFORMER_ROOT%" --checkpoint "%SEGFORMER_CHECKPOINT%" --device "%MASK_DEVICE%" %EXTRA_ARGS%
conda run -n "%SEGFORMER_ENV_NAME%" python waymo_workflow\semantic_masks\cli.py check --data_root "%DATA_ROOT%" --scene_ids "%SCENE_IDS%"
