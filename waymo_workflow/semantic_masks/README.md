...
```bash
bash waymo_workflow/semantic_masks/extract_waymo_masks.sh
```
...
`waymo_workflow/run_waymo_10scenes.sh` checks sky masks before training. If masks are missing, it will try to generate them with the `segformer` environment. Set `AUTO_EXTRACT_MASKS=0` to fail fast instead, or `REQUIRE_SKY_MASKS=0` only for temporary debugging runs.
