...
if "%REQUIRE_SKY_MASKS%"=="1" (
  ...
  conda run -n "%ENV_NAME%" python waymo_workflow\semantic_masks\cli.py check --data_root data\waymo\processed\training --scene_ids "%SCENE_IDS%"
  if errorlevel 1 (
    if not "%AUTO_EXTRACT_MASKS%"=="1" (
      echo [ERROR] Missing sky masks. Run waymo_workflow\semantic_masks\extract_waymo_masks.bat first, or set AUTO_EXTRACT_MASKS=1.
      exit /b 1
    )
    set "MASK_ARGS="
    if "%PROCESS_FINE_DYNAMIC_MASKS%"=="1" set "MASK_ARGS=--process_dynamic_mask"
    conda run -n "%SEGFORMER_ENV_NAME%" python waymo_workflow\semantic_masks\cli.py extract --data_root data\waymo\processed\training --scene_ids "%SCENE_IDS%" --segformer_path "%SEGFORMER_ROOT%" --checkpoint "%SEGFORMER_CHECKPOINT%" --device "%MASK_DEVICE%" !MASK_ARGS!
    conda run -n "%ENV_NAME%" python waymo_workflow\semantic_masks\cli.py check --data_root data\waymo\processed\training --scene_ids "%SCENE_IDS%"
...
if "%REQUIRE_HUMANPOSE%"=="1" (
  ...
  conda run -n "%ENV_NAME%" python waymo_workflow\human_pose\cli.py check --data_root data\waymo\processed\training --scene_ids "%SCENE_IDS%" --smpl_model "%SMPL_MODEL%" !HUMANPOSE_ARGS!
  if errorlevel 1 (
    if "%AUTO_DOWNLOAD_HUMANPOSE%"=="1" (
      ...
      conda run -n "%ENV_NAME%" python waymo_workflow\human_pose\cli.py download-preprocessed --target_dir data --archive_name "%HUMANPOSE_ARCHIVE_NAME%"
    )
    conda run -n "%ENV_NAME%" python waymo_workflow\human_pose\cli.py check --data_root data\waymo\processed\training --scene_ids "%SCENE_IDS%" --smpl_model "%SMPL_MODEL%" !HUMANPOSE_ARGS!
    if errorlevel 1 exit /b 1
  )
