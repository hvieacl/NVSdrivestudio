...
def run_viewer(
    ...
    missing = [p for p in [image_dir, label_dir, kitti_root / "velodyne", kitti_root / "calib"] if not p.exists()]
    if missing:
    ...
    stop = len(dataset) if end_frame < 0 else min(end_frame + 1, len(dataset))
    if start >= stop:
        raise ValueError(f"Empty frame range: start={start}, end={stop}, dataset_len={len(dataset)}")

    viewer = Viewer(box_type="Kitti")
    viewer.set_ob_color_map("gnuplot")
    print(f"Viewing {kitti_root}")
    print(f"Frames: {start}..{stop - 1}; classes: {classes}")
